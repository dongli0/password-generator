...
function detect_any_uppercase($string){
	//true if lowercasing change string
	return strtolower($string) != $string;
}
function detect_any_lowercase($string){
	//true if uppercasing change string
	return strtoupper($string) != $string;
}
function count_number($string){
	return preg_match_all('/[0-9]/',$string);
}
function count_symbols($string){
	//decide which symbols count
...
function password_strength($password){
	...
	$strength +=min(count_number($password),2);
...
<body>
	...
	<form action="" method="post">
		Password: <input type="text" name="rate" value=""></input><br/>
